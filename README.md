...
| <a name="input_enable_pr_pipeline"></a> [enable\_pr\_pipeline](#input\_enable\_pr\_pipeline) | Set `enable_pr_pipeline` to true to create PR pipeline. | `bool` | `true` | no |
...
| <a name="input_pr_worker_id"></a> [pr\_worker\_id](#input\_pr\_worker\_id) | The identifier for the Managed Pipeline worker for the PR pipeline. If not set it will use the value set in `worker_id`. | `string` | `""` | no |
